...
import { buildIndex, searchWithContext, SearchIndex } from '../search/tfidf.js';
...
export function App({ initialState }: AppProps): React.ReactElement {
  ...
  const sendMessage = useCallback(async (text: string): Promise<void> => {
    ...
    if (currentSession.budgetLimit !== null && currentSession.sessionCost >= currentSession.budgetLimit) {
      ...
      const budgetSession: typeof currentSession = { ...currentSession, provider: 'ollama', model: fallback };
    ...
    const run = async (): Promise<void> => {
      ...
      // Inject DNA if set
      const dnaMsg: Message[] = currentSession.dna
        ? [{ role: 'user', content: `[Codebase DNA — your coding style guide]\n${currentSession.dna}` }]
        : [];

      const msgs: Message[] = [...pinnedMsg, ...dnaMsg, ...currentSession.messages, userMsg];
...
<footer>Generated by LocalCode &nbsp;·&nbsp; github.com/TheLocalCodeTeam/localcode</footer>
      ...
      case '/update': {
        ...
        try {
          ...
          const checkUpdate = (): Promise<{ latest: string; current: string; needsUpdate: boolean }> => {
            return new Promise((resolve) => {
              execFile('npm', ['view', '@localcode/cli', 'version'], (err, stdout) => {
                if (err) { resolve({ latest: '', current: pkg.version, needsUpdate: false }); return; }
      ...
      case '/budget': {
        ...
        if (isNaN(amount) || amount <= 0) { sysMsg('Usage: /budget <amount> [fallback-model]', true); return; }
      ...
      case '/routing': {
        ...
        const routing = { planning: null as string | null, execution: null as string | null, review: null as string | null };
        for (const pair of args.split(/\s+/)) {
          const [k, v] = pair.split('=');
          if (k === 'planning' && v) routing.planning = v;
          else if (k === 'execution' && v) routing.execution = v;
          else if (k === 'review' && v) routing.review = v;
        }
      ...
      case '/dna': {
        ...
        try {
          ...
          if (!gitLog.trim()) { sysMsg('⌬  Not enough git history to extract DNA. Make some commits first.', true); setMood('idle'); return; }
          ...
          await streamProvider(session.provider, session.apiKeys, session.model, [{ role: 'user', content: prompt }], (c) => { if (c.text) { dnaText += c.text; updateDisplay(dnaId, { content: dnaText, streaming: true }); } });
        ...
        } catch (err) { sysMsg(`⌬  DNA failed: ${err instanceof Error ? err.message : String(err)}`, true); }
        setMood('idle');
        return;
      }
      ...
      case '/evolve': {
        ...
        let evolved = '';
        await streamProvider(session.provider, session.apiKeys, session.model, [{ role: 'user', content: evolvePrompt }], (c) => { if (c.text) evolved += c.text; });
        if (evolved.trim()) {
      ...
      case '/autopilot': {
        ...
        if (session.autopilotActive) { sysMsg('⬡  Autopilot already running. Use /autopilot off to stop.'); return; }
        ...
        let apDebounce: ReturnType<typeof setTimeout> | null = null;
        autopilotWatcherRef.current = fs.watch(session.workingDir, { recursive: true }, (_event, filename) => {
          ...
          if (apDebounce) clearTimeout(apDebounce);
          apDebounce = setTimeout(async () => {
            try {
              ...
              if (apTestCmd) {
                ...
                shouldCommit = passed;
                if (!passed) { sysMsg(`⬡  Autopilot: ${filename} changed, tests failed — not committing.`); return; }
              }
              if (shouldCommit) {
      ...
      case '/benchmark': {
        ...
        if (targets.length === 0) { sysMsg('No providers configured. Set API keys with /apikey first.', true); return; }
        sysMsg(`⊞  Benchmarking "${benchPrompt.slice(0, 60)}…" across ${targets.length} provider(s)…`);
        setMood('thinking');
        setIsStreaming(true);
        const benchResults: Array<{ provider: string; model: string; latencyMs: number; cost: number; preview: string }> = [];
        try {
